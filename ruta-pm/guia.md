# Guía interna — Ruta PM: skills para tu trabajo de producto
...
Al terminar, el alumno tiene cuatro skills propias de uso diario en su trabajo de PM (discovery, PRD, historias de usuario, métricas), y los primeros documentos reales de una idea de producto suya, generados con esas skills.

**Prerrequisito:** módulo 3 completado (el alumno ya sabe construir una skill: `description` como disparador, formato, restricciones). Esta ruta NO vuelve a enseñar la anatomía de una skill desde cero — se apoya en lo que ya sabe y va más rápido.
...
**Concepto:** recupera el módulo 3 en una frase — *ya sabes construir skills; ahora vas a construir una para tu trabajo real.* Define **discovery** en una frase: *el trabajo de entender el problema del usuario antes de decidir la solución.*

**Construye la skill** `sintetizador-de-entrevistas` a partir de `ruta-pm/starter/skill-plantilla.md`: el alumno decide el disparador y el formato de salida (sugerencia si no se le ocurre: agrupar por tema, citar frases textuales del usuario, separar hechos de opiniones).
...
**Construye la skill** `generador-de-prd`: disparador claro ("cuando pida un PRD borrador a partir de una idea"), formato con secciones mínimas (resumen, problema, usuarios, objetivos, no-objetivos, métricas de éxito) — más ligero que un PRD completo de trabajo, es un borrador de arranque.
...
**Construye la skill** `generador-de-historias`: a partir del PRD, produce 3–5 historias en ese formato, cada una con un criterio de aceptación simple.
...
**Construye la skill** `definidor-de-metricas`: a partir del PRD, produce una métrica principal y 1–2 métricas de apoyo, concretas y medibles (no "mejorar la satisfacción" sino algo que se pueda contar).

**Aplícala** → `mi-trabajo/ruta-pm/metricas.md`.

**Compuerta L5 (checklist de cierre de la ruta):**
- [ ] Las cuatro skills existen en `.claude/skills/`.
- [ ] Los cuatro documentos existen en `mi-trabajo/ruta-pm/` con contenido real sobre la idea del alumno.
- [ ] El alumno puede nombrar al menos una de estas skills que usaría esta semana en su trabajo real.
...
2. Actualiza `mi-trabajo/progreso.md`: Ruta PM completada, fecha, las cuatro skills y documentos.
3. Recapitula en 3 líneas: construiste cuatro skills reutilizables (discovery, PRD, historias, métricas) sobre una idea real tuya — y estas skills siguen viviendo en este repo, listas para reusar en cualquier idea nueva.
