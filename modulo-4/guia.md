...
**Concepto:** recupera la habilidad del módulo 3 como punto de comparación. Una habilidad es una receta que **tú mismo** (el Claude de esta conversación) sigues cuando aplica. Un **agente** es distinto: define en una frase: *un agente es un Claude al que le das un objetivo y las herramientas para lograrlo, y que decide por su cuenta los pasos a seguir, en vez de seguir una receta fija como una habilidad.* Un **subagente** es un agente al que la conversación principal le delega una tarea y que trabaja por su cuenta, aparte, y vuelve con un resultado.
...
**Compuerta L1:** el alumno puede nombrar las cuatro piezas y explicar, en sus palabras, la diferencia entre una habilidad y un agente. Si le cuesta arrancar, dale una frase simple para empezar: "sí, tiene sentido" o "creo que sí, pero no estoy seguro de la parte de...". No hace falta precisión técnica, solo que lo diga con sus palabras.
...
**Setup:** muéstrale `modulo-4/starter/agente-plantilla.md` (misma mecánica que la plantilla de habilidad del módulo 3, pero ahora con las cuatro piezas de la anatomía como secciones marcadas `[AQUÍ VA...]`).

**Guíalo pieza por pieza, recuperando la Lección 1:**

1. **`description`** (el disparador, igual que en habilidades): cuándo debe delegarse trabajo aquí. Ejemplo a afinar con el alumno: "usar cuando pida armar o actualizar el paquete completo de contenido de su sitio."
...
- [ ] El alumno confirmó que reconoció la delegación (otro Claude trabajó la tarea y volvió con el resultado). Frases simples como "sí, eso fue" o "no estoy seguro de qué pasó" son suficientes; no exijas explicación técnica.
...
| Confunde habilidad con agente | Conceptos parecidos | Repite el contraste: habilidad = receta que sigues tú mismo; agente/subagente = otro Claude que hace el trabajo aparte y vuelve con un resultado |
