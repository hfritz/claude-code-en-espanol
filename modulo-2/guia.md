# Guía interna: Módulo 2: Instrucciones que funcionan (prompts)
...
Al terminar, el alumno: (a) puede nombrar las cuatro piezas de una buena instrucción, (b) vivió en carne propia la diferencia entre una instrucción vaga y una estructurada, y (c) tiene `mi-trabajo/sobre-mi.md` (el "sobre mí" de su sitio) escrito con una instrucción que él mismo construyó.
...
## Lección 1: La instrucción vaga (5–10 min)

**Concepto:** define **instrucción (prompt)** en una frase: *lo que le escribes a una IA para pedirle algo; la calidad de lo que recibes depende de la calidad de lo que pides.*

**Ejercicio: el experimento.** Pídele que te escriba, tal cual, sin pensarlo mucho:

`escribe un "sobre mí" para mi sitio personal`

**REGLA CRÍTICA DEL EJERCICIO:** responde usando ÚNICAMENTE lo que dice la instrucción. NO leas `quien-soy.md` todavía. NO uses nada que recuerdes del alumno en esta sesión. El punto pedagógico es que una instrucción sin contexto produce relleno genérico, y eso solo funciona si el resultado ES genérico de verdad ("Soy una persona apasionada por lo que hago..."). Genera 3–4 frases de plantilla vacía.
...
## Lección 2: Anatomía de una instrucción (10–15 min)
...
**Ejercicio:** el alumno arma SU instrucción con las cuatro piezas. Que la escriba él, tú no se la dictas. Si se atasca: pista 1 = recuérdale las cuatro piezas; pista 2 = muéstrale UNA pieza de ejemplo escrita. Acepta variaciones razonables; corrige solo si falta una pieza completa (típico: olvidan formato o tono, señálalo con una pregunta: "¿de qué largo lo quieres?").

**Ejecución:** cuando la instrucción tenga las cuatro piezas, ejecútala DE VERDAD: lee `quien-soy.md` y escribe un "sobre mí" genuinamente bueno con su material. Este contraste es el corazón del módulo: el resultado debe ser notablemente mejor que el de la Lección 1.
...
**Compuerta L2:** el alumno escribió una instrucción con las cuatro piezas (con ayuda está bien) y puede nombrarlas de memoria o casi. Pídele que las diga.
...
3. Recapitula en 3 líneas: instrucción vaga = relleno; cuatro piezas (tarea, contexto, formato, tono); el contexto fue la reina.
4. Anticipa el módulo 3 en una frase: *escribiste una gran instrucción una vez, ¿y si Claude la recordara para siempre? Eso es una habilidad (skill).* Sin más detalle.
...
|---------|----------------|-----------|
| El resultado "vago" de L1 salió bueno | No respetaste la regla crítica (usaste contexto de la sesión) | No debe pasar. Si pasó, reconócelo con humor ("me conoces, hice trampa") y rehazlo solo con la instrucción literal |
| No existe `quien-soy.md` | Saltó el módulo 1 o borró el archivo | Prerrequisito del inicio: `/modulo-1` o mini-entrevista exprés |
| El alumno pide que TÚ escribas su instrucción | Inseguridad | Pista, no solución: dale la primera pieza escrita y que complete las otras tres |
| Instrucción de una línea sin piezas ("hazlo mejor") | No interiorizó la anatomía | Pregunta por la pieza faltante en vez de corregir: "¿qué contexto le estás dando?" |
